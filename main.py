import pandas as pd
from seismic import pred_cascade, get_infectiousness
import numpy as np
...
def main(num_preds=100, num_inf_preds=1000):
    ...
    index_df = pd.read_csv('data/index.csv', dtype={'start_ind': np.int32, 'end_ind': np.int32})
    ...
    for i, row in index_df.iterrows():
        ...
        p_time = np.linspace(cascade_df['relative_time_second'].iloc[1], max_time, num=num_inf_preds)
        ...
        plot_cascade(share_time, p_time, pred_total, smooth_pred, fname=f'prediction{i}.jpg')
