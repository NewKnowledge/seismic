...
df <- read.csv("test_input.csv")
inf <- get.infectiousness(df$share_time, df$degree, df$p_time)
pred <- pred.cascade(df$p_time, inf$infectiousness, df$share_time, df$degree)
new_df <- cbind.data.frame(inf, data.frame(pred))
write.csv(new_df, 'r_output.csv')
